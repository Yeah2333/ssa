...
namespace ssa {
    ...
    class SSA3DglWidget : public QGLViewer {
    Q_OBJECT

    public:
        SSA3DglWidget();

        SSA3DglWidget(QWidget *&widget);
        ...
        */
        ...
        inline void setMap(SparseSurfaceAdjustmentGraph3D *accurateMap) {
    ...
    public slots:
        ...
        inline void setPointSize(double i) {
            _pointSize = i;
            updateGL();
            cerr << "pointSize " << _pointSize << endl;
            Gen3DObjectList_update();
        };

        inline void setDrawGrid(int i) {
            _drawGrid = (bool) i;
            updateGL();
        }

        inline void setDrawVertices(int i) {
            _drawVertices = (bool) i;
            updateGL();
        }

        inline void setDrawLandmarkVertices(int i) {
            _drawLandmarkVertices = (bool) i;
            updateGL();
        }

        inline void setDrawNormals(int i) {
            _drawNormals = (bool) i;
            updateGL();
        }

        inline void setDrawCorrespondences(int i) {
            _drawCorrespondences = (bool) i;
            updateGL();
        }

        inline void setDrawMesh(int i) {
            _drawMesh = (bool) i;
            updateGL();
        }
    ...
    public:
        QWidget *_parent;
        SparseSurfaceAdjustmentGraph3D *_ssa_graph;
