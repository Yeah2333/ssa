#include "ssa_3d_qglviewer_widget.h"
#include <QFileDialog>
...
namespace ssa {
    ...
    SSA3DglWidget::SSA3DglWidget(QWidget *&widget) {
...
//   _initCalled = true;
    ...
    void SSA3DglWidget::drawWithNames() {
        ...
        for (g2o::OptimizableGraph::VertexIDMap::const_iterator it = _ssa_graph->_optimizer.vertices().begin();
            ...
            if (v) {
                glPushName(v->id());
                glPushMatrix();
                glTranslatef(v->estimate().translation()(0), v->estimate().translation()(1),
                             v->estimate().translation()(2));
    ...
    void SSA3DglWidget::drawEdges() {
        ...
        if (_drawCorrespondences)
            for (unsigned int i = 0; i < _ssa_graph->_edges_data_association.size(); ++i) {
                EdgePointXYZCovPointXYZCov *&edge = _ssa_graph->_edges_data_association[i];
                VertexPointXYZCov *from = dynamic_cast<VertexPointXYZCov * >(edge->vertices()[0]);
                VertexPointXYZCov *to = dynamic_cast<VertexPointXYZCov * >(edge->vertices()[1]);
                glBegin(GL_LINES);
...
//   }
    ...
    void SSA3DglWidget::drawMeshEdges() {
        for (unsigned int i = 0; i < _ssa_graph->_edges_surface_mesh.size(); ++i) {
            glColor3f(0.0, 0.0, 1.0);
            EdgePointXYZCovPointXYZCov *&edge = _ssa_graph->_edges_surface_mesh[i];
            VertexPointXYZCov *from = dynamic_cast<VertexPointXYZCov * >(edge->vertices()[0]);
            VertexPointXYZCov *to = dynamic_cast<VertexPointXYZCov * >(edge->vertices()[1]);
    ...
    void SSA3DglWidget::drawCovariance() {
//   for(unsigned int i = 0; i < _ssa_graph->_meshEdges.size(); ++i){
//      EdgePointXYZCovPointXYZCov*& edge = _ssa_graph->_meshEdges[i];
//      VertexPointXYZCov* from = dynamic_cast<VertexPointXYZCov* >(edge->vertices()[0]);
...
//   }
    ...
    void SSA3DglWidget::drawEllipsoid(double &l1, double &l2, double &l3) {
    ...
    void SSA3DglWidget::drawCircle(GLfloat radius, int segments) {
    ...
    void SSA3DglWidget::drawPoseBox() {
        glPushMatrix();
        glScalef(0.5, 1, 1);
        glPushMatrix();
        glScalef(1, 0.25, 0.5);
    ...
    void SSA3DglWidget::drawBox(GLfloat l, GLfloat w, GLfloat h) {
        GLfloat sx = l * 0.5f;
        GLfloat sy = w * 0.5f;
        GLfloat sz = h * 0.5f;
    ...
    void SSA3DglWidget::postSelection(const QPoint &point) {
        if (selectedName() != -1) {
            qglviewer::Vec orig;
            qglviewer::Vec dir;
    ...
    void SSA3DglWidget::setOptimize() {
        _optimize = true;
    }


    void SSA3DglWidget::openFile() {
    ...
    GLint SSA3DglWidget::Gen3DObjectList_poseVertices() {
    ...
    void SSA3DglWidget::drawVertices() {
        // Draw vertices
        ...
        for (size_t i = 0; i < indices.size(); ++i) {
            ...
            for (std::vector<ssa::VertexPointXYZCov *>::const_iterator it = vertices.begin();
                 it != vertices.end(); it++) {
    ...
    GLint SSA3DglWidget::Gen3DObjectList_obsVertices() {
        GLint lid = glGenLists(1);
        glNewList(lid, GL_COMPILE);
        glPointSize(_pointSize * 100);
    ...
    GLint SSA3DglWidget::Gen3DObjectList_normals() {
        ...
        if (_ssa_graph)
            for (g2o::OptimizableGraph::VertexIDMap::const_iterator it = _ssa_graph->_optimizer.vertices().begin();
                 it != _ssa_graph->_optimizer.vertices().end(); it++) {
                VertexPointXYZCov *v = dynamic_cast<VertexPointXYZCov *>(it->second);
                if (v) {
...
//   }
    ...
    GLint SSA3DglWidget::Gen3DObjectList_mesh() {
        GLint lid = glGenLists(1);
        glNewList(lid, GL_COMPILE);
        glPointSize(_pointSize * 100);
...
//         continue;
    ...
    void SSA3DglWidget::Gen3DObjectList_update() {
        glDeleteLists(_poseVertices, 1);
        glDeleteLists(_obsVertices, 1);
...
//   _mesh = Gen3DObjectList_mesh();
    ...
    void SSA3DglWidget::saveSnapshotCustom() {
    ...
    void SSA3DglWidget::saveSnapshotVideo() {
        QDateTime current;
        char fileName[2048];
