...
namespace ssa {
    ...
    KDTreeFlannT<PointVertexType>::KDTreeFlannT() : data_(0), dim_(PointVertexType::Dimension), flann_index_(0),
                                                    mode_(0) {
    };
    ...
    template<typename PointVertexType>
    void
    KDTreeFlannT<PointVertexType>::copyData(std::vector<PointVertexType *> &vertices, bool useVectorIndices = false) {
...
//     double timing = get_time();
        ...
        flann_index_ = new flann::Index<flann::L2_Simple<float> >(
                flann::Matrix<float>(data_, index_mapping_.size(), dim_), flann::KDTreeSingleIndexParams(10));
...
//     std::cerr << "FLANN kDTree construction took " << get_time() - timing << "ms."<< std::endl;
    ...
    void KDTreeFlannT<PointVertexType>::nearestKSearch(const PointVertexType *vertex, int k,
        ...
        // do a knn search
        flann_index_->knnSearch(flann::Matrix<float>(&tmp[0], 1, dim_), k_indices_mat, k_distances_mat, k,
    ...
    template<typename PointVertexType>
    int KDTreeFlannT<PointVertexType>::radiusSearch(const PointVertexType *vertex, double radius,
                                                    std::vector<int> &k_indices,
        ...
        neighbors_in_radius = flann_index_->radiusSearch(flann::Matrix<float>(&tmp[0], 1, dim_),
                                                         ...
                                                         flann::SearchParams(-1, 0.0001, true));
    ...
    int KDTreeFlannT<PointVertexType>::neighborsInRadius(const PointVertexType *vertex, double radius) const {
        ...
        int neighbors_in_radius = flann_index_->radiusSearch(flann::Matrix<float>(&tmp[0], 1, dim_),
                                                             ...
                                                             flann::SearchParams(-1, 0.0001, false));
        return neighbors_in_radius;
    }
    ...
    template<typename PointVertexType>
    void
    KDTreeFlannT<PointVertexType>::vectorize(unsigned int dim, const PointVertexType *vertex, std::vector<float> &vec) {
...
//         vec[i] = (float) vertex->normal()[i-3];
    ...
    template<typename PointVertexType>
    void
    KDTreeFlannT<PointVertexType>::vectorize(unsigned int dim, const Eigen::Vector3f &point, std::vector<float> &vec) {
        for (size_t i = 0; i < dim; ++i)
            vec[i] = point(i);
    }
