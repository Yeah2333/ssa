...
namespace ssa {
    ...
    SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::SparseSurfaceAdjustmentGraphT() : maxVertexId(0),
                                                                                                      max_level_(0) {
    }
...
#pragma omp critical
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::load(std::string filename) {
        ...
        if (!graphstream) {
            std::cerr << "SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::load( " << filename << ")"
                      << ": error in loading the graph." << std::endl;
            return;
        }
        _optimizer.load(graphstream);
        std::cerr << "loaded ssa graph with " << _optimizer.vertices().size() << " vertices and "
                  << _optimizer.edges().size() << " edges." << std::endl;
        linkNodesToVertices();
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::save(std::string filename) {
        ...
        if (!graphstream) {
            std::cerr << "SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::save( " << filename << ")"
                      << ": error while opening the target file." << std::endl;
            return;
        }
        std::cout << 1 << std::endl;
...
//     }
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::linkNodesToVertices() {
        ...
        for (g2o::OptimizableGraph::VertexIDMap::const_iterator it = _optimizer.vertices().begin();
        ...
        for (g2o::OptimizableGraph::EdgeSet::iterator it = _optimizer.edges().begin();
            ...
            if (e1) {
                ...
                if (e2) {
                    ...
                    chi2 = e2->chi2();
                    if (chi2 < 0.0) {
                        std::cerr << "Error: NEGATIVE INFORMATION MATRIX... THIS SHOULD NOT HAPPEN..."
                                  << " SensorEdgeType " << e2->vertices()[0]->id() << " " << e2->vertices()[1]->id()
                                  << " " << chi2 << std::endl;
                    ...
                    if (e3) {
                        ...
                        if (chi2 < 0.0) {
                            std::cerr << "Error: NEGATIVE INFORMATION MATRIX... THIS SHOULD NOT HAPPEN... "
                                      << e3->vertices()[0]->id() << " " << e3->vertices()[1]->id() << " " << std::endl;
...
//               exit(-1);
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::fillObservations() {
        ...
        _edges_observations.clear();
        //typename
        for (typename std::vector<PoseVertex *>::iterator it = _verticies_poses.begin();
             it != _verticies_poses.end(); it++) {
            PoseVertex *&v = (*it);
            if (v) {
                for (g2o::OptimizableGraph::EdgeSet::iterator itt = v->edges().begin();
                     itt != v->edges().end(); itt++) {
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    void
    SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::calcMeanCov(SparseSurfaceAdjustmentParams &params) {
...
#pragma omp parallel for shared(params, keys)
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    void
    SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::fillNeighborCache(
    ...
    void
    SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::fillNeighborCache(
            SparseSurfaceAdjustmentParams &params, int scanId) {
        Observation<PointVertex> &observation = getObservationOfScan(scanId);
...
//   }
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    int SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::addVertex(PointVertex *&v,
                                                                                  std::vector<PointVertex *> &neighbors) {
        int id = addVertex(v);
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::removeVertex(PoseVertex *&v) {
        ...
        typename std::vector<PoseVertex *>::iterator it = std::find(_verticies_poses.begin(), _verticies_poses.end(),
                                                                    v);
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::removeVertex(PointVertex *&v) {
        ...
        typename std::vector<PointVertex *>::iterator itt = std::find(_verticies_points.begin(),
                                                                      _verticies_points.end(), v);
...
//     std::cerr << _edges_data_association_gicp.size() << std::endl;
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::addEdge(PointVertex *&from, PointVertex *&to) {
        ...
        for (g2o::OptimizableGraph::EdgeSet::iterator it = from->edges().begin(); it != from->edges().end(); it++) {
            ...
            if (e1) {
                ...
                if ((v1->id() == from->id() && v2->id() == to->id()) ||
                    (v1->id() == to->id() && v2->id() == from->id())) {
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    typename SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::DataAssociationEdgeType *
    SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::createEdge(PointVertex *&from, PointVertex *&to) {
        if (from == to) {
            std::cerr << "Warning: you tried to create an edge between an observation and itself." << std::endl;
            return 0;
        }
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::removeEdge(SLAMEdgeType *edge) {
        ...
        typename std::vector<SLAMEdgeType *>::iterator it = std::find(_edges_odometry.begin(), _edges_odometry.end(),
                                                                      edge);
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::removeEdge(SensorEdgeType *edge) {
        ...
        typename std::vector<SensorEdgeType *>::iterator it = std::find(_edges_observations.begin(),
                                                                        _edges_observations.end(), edge);
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::removeEdge(DataAssociationEdgeType *edge) {
        ...
        typename std::vector<DataAssociationEdgeType *>::iterator it = std::find(_edges_data_association.begin(),
                                                                                 _edges_data_association.end(), edge);
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::setFixedVertices(bool fixed) {
        ...
        for (g2o::OptimizableGraph::VertexIDMap::iterator it = _optimizer.vertices().begin();
             it != _optimizer.vertices().end(); it++) {
...
//     _edges_data_association.clear();
    ...
    void SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::pruneGraph(double minDistance) {
        ...
        for (typename std::map<double, DataAssociationEdgeType *>::iterator it = da_edges.begin();
             it != da_edges.end(); ++it) {
...
//     std::vector<DataAssociationEdgeType* > edgesToRemove;
        ...
        for (size_t i = 0; i < _verticies_points.size(); ++i) {
            ...
            if (vp) {
                ...
                for (g2o::OptimizableGraph::EdgeSet::iterator itt = vp->edges().begin();
                     itt != vp->edges().end(); itt++) {
                ...
                count = 0;
                for (typename std::deque<DataAssociationEdgeType *>::const_iterator it = edgesOfVertex.begin();
                     it != edgesOfVertex.end(); ++it) {
                    DataAssociationEdgeType *e2 = (*it);
                    if (e2) {
                        if (count >= maxEdges && (int) e2->vertices()[0]->edges().size() > (maxEdges + 1) &&
                            (int) e2->vertices()[1]->edges().size() > (maxEdges + 1)) {
...
//     }
                ...
                if (vp) {
                    for (g2o::OptimizableGraph::EdgeSet::iterator itt = vp->edges().begin();
                        ...
                        if (e2) {
                            ...
                            if (vp2) {
                                for (g2o::OptimizableGraph::EdgeSet::iterator ittt = vp2->edges().begin();
                                     ittt != vp2->edges().end(); ittt++) {
    ...
    bool SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::connected(PointVertex *&v1, PointVertex *&v2) {
        for (g2o::OptimizableGraph::EdgeSet::iterator it = v1->edges().begin(); it != v1->edges().end(); it++) {
            ...
            if (e2) {
                if ((e2->vertices()[0] == v1 && e2->vertices()[1] == v2) ||
                    (e2->vertices()[0] == v2 && e2->vertices()[1] == v1))
                    return true;
            }
        }
        return false;
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    bool SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3>::haveCommonSensorVertex(PointVertex *&v1,
                                                                                                PointVertex *&v2) {
        if (v1->id() == v2->id())
            return true;
