...
namespace ssa {
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    void
    SparseSurfaceAdjustmentT<EdgeType1, EdgeType2, EdgeType3>::setGraph(
            SparseSurfaceAdjustmentT<EdgeType1, EdgeType2, EdgeType3>::SSAGraph &graph) {
        graph_ = graph;
    }
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    void SparseSurfaceAdjustmentT<EdgeType1, EdgeType2, EdgeType3>::setSolver(
            g2o::BlockSolverX::LinearSolverType *&linearSolver) {
        g2o::BlockSolver<BlockSolverTraits<-1, -1> > *blockSolver = new g2o::BlockSolver<BlockSolverTraits<-1, -1> >(
                linearSolver);
...
//         da_kdtree.apply(graph_, params(), level);
            ...
            cerr << "Edge set construction of size " << eset.size() << " took " << (get_time() - timing) * 1000 << " ms"
                 << endl;
    ...
    void SparseSurfaceAdjustmentT<EdgeType1, EdgeType2, EdgeType3>::dumpConnectivityMatrix() {
        ...
        for (g2o::OptimizableGraph::EdgeSet::iterator it = graph()->_optimizer.edges().begin();
             it != graph()->_optimizer.edges().end(); it++) {
