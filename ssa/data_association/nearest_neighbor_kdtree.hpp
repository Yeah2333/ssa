...
template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
void
NearestNeighborKdtreeT<EdgeType1, EdgeType2, EdgeType3>::apply(ScanPairVector scanPairs,
                                                               CorrespondenceList &resultingCorrespondences,
                                                               SparseSurfaceAdjustmentGraphT <EdgeType1, EdgeType2, EdgeType3> &graph,
                                                               SparseSurfaceAdjustmentParams &params, int level = 0) {
...
#pragma omp critical
    ...
    std::cerr << "NearestNeighborKdtreeT pre calculation took " << (get_time() - timing) * 1000.0 << "ms with "
              << numKdTreeThreads << " threads." << std::endl;
    timing = get_time();
    ...
    for (int j = 0; j < (int) scanPairs.size(); ++j) {
        ...
        for (size_t k = 0; k < vertices_id2.size(); k = k + params.nearestNeighbor.increment) {
            ...
            if (k_squared_distances[0] < sqrDistance) {
                assign(correspondencesPerThread[threadId], vertices_id2[k], vertices_id1[k_indices[0]],
                       k_squared_distances[0]);
            }
        }
    }
