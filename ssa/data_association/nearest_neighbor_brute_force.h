...
namespace ssa {
    ...
    public:
        ...
        void apply(CorrespondenceList &resultingCorrespondences,
                   SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3> &graph,
                   SparseSurfaceAdjustmentParams &params, int level);

        /** \brief creates data association edges for all available scan pairs  */
        void apply(OverlapMap overlap, CorrespondenceList &resultingCorrespondences,
                   SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3> &graph,
                   SparseSurfaceAdjustmentParams &params, int level);

        /** \brief creates data association edges for all available scan pairs  */
        virtual void apply(ScanPairVector scanPairs, CorrespondenceList &resultingCorrespondences,
                           SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3> &graph,
                           SparseSurfaceAdjustmentParams &params, int level);

        /** \brief creates data association edges for all available scan pairs  */
        void applyForScanPair(int id1, int id2, CorrespondenceList &resultingCorrespondences,
                              SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3> &graph, double sqrDistance,
                              int level);
        ...
        ScanPairVector
        getScanPairs(SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3> &graph, OverlapMap overlap,
                     float minOverlap);

        void assign(CorrespondenceList &resultingCorrespondences, PointVertex *&reference, PointVertex *&correspondence,
                    float distance);

        /** \brief creates an edge between reference and correspondence */
        void assign(SparseSurfaceAdjustmentGraphT<EdgeType1, EdgeType2, EdgeType3> &graph, PointVertex *&reference,
