...
namespace ssa {
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    void
    NormalShootingFlann<EdgeType1, EdgeType2, EdgeType3>::shootNormals(
            SparseSurfaceAdjustmentGraphT <EdgeType1, EdgeType2, EdgeType3> &graph, NormalShootingParams &params) {
        if (params.steps < 2) {
            cerr << __PRETTY_FUNCTION__ << ": Warning params.steps is set to value < 1. Skipping normal shooting."
                 << endl;
            return;
        }
...
#pragma omp parallel for schedule(dynamic, 1) shared(graph, kDTree, params)
    ...
    NormalShootingFlann<EdgeType1, EdgeType2, EdgeType3>::shootNormals(
            ...
            NormalShootingFlann<EdgeType1, EdgeType2, EdgeType3>::PointTree &kDTree, int &scanId,
            NormalShootingParams &params) {
        ...
        for (unsigned int k = 0; k < scan.size(); k = k + params.increment) {
            ...
            for (int l = -params.steps; l < params.steps; ++l) {
                ...
                if (kDTree.radiusSearch(vp, params.stepSize, k_indices, k_squared_distances, 1) != 0) {
                    ...
                    if (CorrespondenceRejectionT<EdgeType1, EdgeType2, EdgeType3>::isValid(point, correspondence,
                                                                                           params.maxAngleDifference,
                                                                                           params.maxColorChannelDiff)) {
...
//             }
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    void
    NormalShootingFlann<EdgeType1, EdgeType2, EdgeType3>::normalOutlierReject(
            SparseSurfaceAdjustmentGraphT <EdgeType1, EdgeType2, EdgeType3> &graph, NormalShootingParams &params) {
        if (params.steps < 2) {
            cerr << __PRETTY_FUNCTION__ << ": Warning params.steps is set to value < 1. Skipping normal shooting."
                 << endl;
