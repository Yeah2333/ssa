...
namespace ssa {
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    bool
    CorrespondenceRejectionT<EdgeType1, EdgeType2, EdgeType3>::isValid(PointVertex *&reference,
    ...
    CorrespondenceRejectionT<EdgeType1, EdgeType2, EdgeType3>::isValid(PointVertex *&reference,
        ...
        if (correspondence->id() == reference->id())
            return false;
        if (reference->covariance() == PointMatrix::Identity() ||
    ...
    template<typename EdgeType1, typename EdgeType2, typename EdgeType3>
    bool
    CorrespondenceRejectionT<EdgeType1, EdgeType2, EdgeType3>::isValid(
            CorrespondenceT <PointVertex, EdgeType3> &correspondence, double &maxAngleDifference,
            double &maxColorChannel) {
        ...
        if (correspondence.cor_point->parentVertexId() == correspondence.query_point->parentVertexId())
            return false;
        if (correspondence.query_point->covariance() == PointMatrix::Identity() ||
            correspondence.cor_point->covariance() == PointMatrix::Identity())
            return false;

        double angleBetweenNormals = fabs(
                acos(correspondence.query_point->globalNormal().dot(correspondence.cor_point->globalNormal())));
        if (angleBetweenNormals >= DEG2RAD(maxAngleDifference))
            return false;
        if (maxColorChannel == 1.0)
            return true;
