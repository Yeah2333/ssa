...
void evaluateResult(
        NearestNeighborBruteForceT<g2o::EdgeSE3, ssa::EdgeSE3PointXYZCov, ssa::EdgePointXYZCovPointXYZCov>::CorrespondenceList &correspondences,
    ...
    for (size_t i = 0; i < correspondences.size(); ++i) {
        ...
        if (correspondence && !reference_correspondence) {
            std::cerr << "Warning no reference found for correspondence " << (correspondence->query_point->id())
                      << " -> " << (correspondence->cor_point->id()) << " "
                      << (correspondence->query_point->estimate() - correspondence->cor_point->estimate()).squaredNorm()
                      << std::endl;
        }

        if (correspondence && reference_correspondence) {
            possibleMatches++;
            bool almostSimilarDistance = fabs(correspondence->sqrDistance - reference_correspondence->sqrDistance) <
...
//           std::cerr << PVAR(correspondence->cor_point->id()) << " " << PVAR(reference_correspondence->cor_point->id()) << std::endl;
            ...
            if (correspondence->cor_point->id() == reference_correspondence->cor_point->id() &&
                correspondence->query_point->id() == reference_correspondence->query_point->id()) {
                matches++;

                float trueDistance = (correspondence->cor_point->estimate() -
                                      correspondence->query_point->estimate()).squaredNorm();
...
//         std::cerr << "ERROR: " << (correspondence->query_point->id()) << " -> " <<(correspondence->cor_point->id()) << "(" << correspondence->sqrDistance << ")" << " vs. reference " << (reference_correspondence->query_point->id()) << " -> " << (reference_correspondence->cor_point->id())  << "(" << reference_correspondence->sqrDistance << ")" << std::endl;
    ...
    cerr << "Match percentage   \t " << ((float) possibleMatches / (float) correspondences.size()) * 100.0f << "% "
         << std::endl;
    ...
    cerr << "GT distance matches\t " << ((float) groundTruthDistMatches / (float) possibleMatches) * 100.0f << "% "
         << std::endl;
}
...
//       debug=true;
    ...
    da_brute_force.setStrategy(
            DataAssociationT<g2o::EdgeSE3, ssa::EdgeSE3PointXYZCov, ssa::EdgePointXYZCovPointXYZCov>::BRUTEFORCE);
    da_brute_force.applyBasedOnOverlap(ssaGraph, params, 0);
    std::cerr << " [done] " << da_brute_force.correspondences.size() << " results in " << (get_time() - timing) * 1000.0
              << "ms." << std::endl;
    ...
    da_kdtree.setStrategy(
            DataAssociationT<g2o::EdgeSE3, ssa::EdgeSE3PointXYZCov, ssa::EdgePointXYZCovPointXYZCov>::KDTREE);
    da_kdtree.apply(ssaGraph, params, 0);
    std::cerr << " [done] " << da_kdtree.correspondences.size() << " results in " << (get_time() - timing) * 1000.0
              << "ms." << std::endl;
    ...
    da_sparse_kdtree_local.setStrategy(
            DataAssociationT<g2o::EdgeSE3, ssa::EdgeSE3PointXYZCov, ssa::EdgePointXYZCovPointXYZCov>::SPARSE_KDTREE_LOCAL);
    da_sparse_kdtree_local.apply(ssaGraph, params, 0);
    std::cerr << " [done] " << da_sparse_kdtree_local.correspondences.size() << " results in "
              << (get_time() - timing) * 1000.0 << "ms." << std::endl;
