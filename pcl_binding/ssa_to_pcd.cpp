...
int main(int argc, char **argv) {
    ...
    if (reproject) {
        cloud = PCLSSAHierarchicalT<pcl::PointCloud<pcl::PointXYZRGBA> >::landmarksToPointCloud(
                ssaGraph._edges_observations);
    } else {
        int i = 0;
        std::map<int, int> keys = ssaGraph.getScanIds();
        for (std::map<int, int>::iterator it = keys.begin(); it != keys.end(); ++it) {
            int scanId = it->first;
            pcl::PointCloud<pcl::PointXYZRGBA> tmp = PCLSSAHierarchicalT<pcl::PointCloud<pcl::PointXYZRGBA> >::landmarksToPointCloud(
                    ssaGraph._verticies_observations[scanId]);
