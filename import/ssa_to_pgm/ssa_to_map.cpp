...
int main(int argc, const char **argv) {
    ...
    while (c < argc) {
        if (!strcmp(argv[c], "-res")) {
            ...
            c++;
        } else if (!strcmp(argv[c], "-maxrange")) {
            c++;
            maxRange = atof(argv[c]);
            c++;
        } else if (!strcmp(argv[c], "-usablerange")) {
            c++;
    ...
    for (g2o::OptimizableGraph::EdgeSet::iterator it = ssa.graph()->_optimizer.edges().begin();
         it != ssa.graph()->_optimizer.edges().end(); it++) {
        ...
        if (edge) {
            ...
            if (vp1 && vp2) {
                Eigen::Vector2f rp = Eigen::Vector2f(vp1->estimate().translation()(0),
                                                     vp1->estimate().translation()(1));
