...
namespace ssa {
    ...
    void SSAPoseGraph2D::importPoseGraph2D(std::string logfile, SparseSurfaceAdjustmentGraph2D &ssaGraph) {
        ...
        while (is) {
            ...
            if (tag == "VERTEX" || tag == "VERTEX2" || tag == "VERTEX_SE2") {
                ...
                if (!v1) {
                    cerr << "vertex " << id1 << " is not existing, cannot add edge (" << id1 << "," << id2 << ")"
                         << endl;
                    continue;
                }
                if (!v2) {
                    cerr << "vertex " << id2 << " is not existing, cannot add edge (" << id1 << "," << id2 << ")"
                         << endl;
                    continue;
                }
    ...
    void
    SSAPoseGraph2D::importRobotLaser(std::istream &is, VertexSE2 *parent, SparseSurfaceAdjustmentGraph2D &ssaGraph) {
        int type;
